...
def extract_customer_name(text: str, known_customers: list[str]) -> Optional[str]:
    """
    Kullanıcının verdiği cümlede, veritabanındaki bilinen müşteri isimlerinden birini bulur.
    """
    text_lower = text.lower()
    for customer in known_customers:
        if customer.lower() in text_lower:
            return customer  # Tam eşleşen müşteri adını döndür
